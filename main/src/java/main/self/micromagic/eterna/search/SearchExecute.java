		...
		implements Execute, Generator
{
			...
			throws EternaException
	{
		...
		this.sql_get_setting_id = this.factory.findObjectId("eterna.search.get.search_setting");
		this.sql_delete_setting_id = this.factory.findObjectId("eterna.search.delete.search_setting");
		this.sql_add_setting_id = this.factory.findObjectId("eterna.search.insert.search_setting");
	}
			...
			throws EternaException, SQLException, IOException
	{
		...
		else if ("save".equals(action))
		{
			...
			while (settingXML.length() > 0)
			{
				...
				update.setObject(4, Utility.createInteger(index));
