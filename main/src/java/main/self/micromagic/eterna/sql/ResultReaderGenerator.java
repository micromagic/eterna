...
public interface ResultReaderGenerator extends Generator
{
	 ...
	 * 的列名后就不能再设置使用索引值.
	 */
	void setColumnName(String columnName) throws EternaException;
	 ...
	 * 的索引值后就不能再设置使用列名.
	 */
	void setColumnIndex(int columnIndex) throws EternaException;

	/**
	 * 设置作为排序列, 即出现在"ORDER BY"之后的列名.
	 * 在多个表时, 也可以是"[表名].[列名]"的形式.
	 */
	void setOrderName(String orderName) throws EternaException;

	/**
	 * 设置format的名称.
	 */
	void setFormatName(String name) throws EternaException;
	 ...
	 */
	void setHtmlFilter(boolean filter) throws EternaException;
	 ...
	 */
	ResultReader createReader() throws EternaException;
